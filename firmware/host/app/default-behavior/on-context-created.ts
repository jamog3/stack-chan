...
import { startVbusShutdownWatch } from 'vbus-shutdown'
...
export const onContextCreated: NonNullable<StackchanAppBehavior['onContextCreated']> = (robot) => {
  ...
  // USB power-loss shutdown. Logic lives in its own module (see vbus-shutdown.ts) so
  // upstream churn in this already-enormous function doesn't collide with it.
  startVbusShutdownWatch(robot)
  robot.drawer.addDrawerButton({
